...
admin.initializeApp();
exports.onTaskCreatedSendNotifications = (0, firestore_1.onDocumentCreated)("contentSchedule/{contentId}", // Ruta de la colección
async (event) => {
    if (!event.data) {
    ...
    // Mueve la lectura de la clave y la creación del transportador aquí dentro
    // para que solo se inicialice cuando la función se ejecuta.
    const SENDGRID_API_KEY = functions.config().sendgrid.key; // Se lee aquí
    ...
    // FIN de mover la lectura de clave y creación del transportador
